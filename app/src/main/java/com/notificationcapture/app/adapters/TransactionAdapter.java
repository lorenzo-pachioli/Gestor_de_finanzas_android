...
public class TransactionAdapter extends RecyclerView.Adapter<TransactionAdapter.ViewHolder> {
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        ...
        if (item.isExpanded()) {
            bindExpandedView(holder, item, context);
        } else {
            bindCollapsedView(holder, item);
        }
    }

    private void bindExpandedView(ViewHolder holder, Transaction item, Context context) {
        ...
        holder.currentWatcher = new MoneyTextWatcher(holder.getEtAmount());
        holder.getEtAmount().addTextChangedListener(holder.currentWatcher);

        boolean isIngreso = item.getType() == IngresoOEgreso.INGRESO;
        holder.getSwitchType().setChecked(!isIngreso, true);

        Category currentCat = resolveCategory(item.getCategoryId(), item.getType());
        ...
        holder.getSwitchType().setOnCheckedChangeListener(isChecked -> {
            ...
            resetCategoryForType(holder, !isChecked);
        });

        holder.getBtnSave().setOnClickListener(v -> saveItem(holder, item));
    ...
    private void bindCollapsedView(ViewHolder holder, Transaction item) {
        ...
        boolean hasText = item.getText() != null && !item.getText().trim().isEmpty();
        holder.getTvText().setVisibility(hasText ? View.VISIBLE : View.GONE);
        if (hasText) holder.getTvText().setText(item.getText());

        holder.getTvNotificacion().setVisibility(item.isNotification() ? View.VISIBLE : View.GONE);
        holder.getTvTimestamp().setText(dateFormat.format(new Date(item.getTimestamp())));
        ...
        holder.getBtnDelete().setOnClickListener(v ->
            Dialog.show("¿Estás seguro de que deseas eliminar esta transacción?",
    ...
    private void resetCategoryForType(ViewHolder holder, boolean isIngreso) {
        ...
        for (Category c : newTypeCategories) {
            if (c.getName().toLowerCase().equals(currentName)) {
                found = c;
                break;
            }
        }
    ...
    private void saveItem(ViewHolder holder, Transaction item) {
        ...
        item.setAmount(MoneyTextWatcher.parse(holder.getEtAmount().getText().toString()));
        item.setExpanded(false);
    ...
    private void showPaymentMethodSelector(Context context, ViewHolder holder, Transaction item) {
        ...
        bottomSheet.setListener((method, detailId, installments) -> {
            ...
            BigDecimal currentAmount = MoneyTextWatcher.parse(holder.getEtAmount().getText().toString());
